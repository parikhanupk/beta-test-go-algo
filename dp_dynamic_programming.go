...
func dynamic_programming(items []Item, allowed_weight int) ([]Item, int, int) {
    ...
    for i := 0; i < num_items; i++ {
        solution_value[i] = make([]int, allowed_weight + 1)
        prev_weight[i]    = make([]int, allowed_weight + 1)
    }
    ...
    for i := 1; i < num_items; i++ {
        for w := 0; w <= allowed_weight; w++ {
            ...
            value_without_i := solution_value[i - 1][w]

            // Calculate the value if we do use the new item i.
            value_with_i := 0
            if items[i].weight <= w {    // Make sure it fits.
                value_with_i = solution_value[i - 1][w - items[i].weight] + items[i].value
            }
    ...
    for i >= 0 {
        ...
        prev_w := prev_weight[i][w]
        if w == prev_w {
            // We skipped item i.
            // Leave w unchanged.
        } else {
            // We added item i.
            items[i].is_selected = true   // Select this item in the solution.
            w = prev_w                    // Move to the previous solution's weight.
        }
